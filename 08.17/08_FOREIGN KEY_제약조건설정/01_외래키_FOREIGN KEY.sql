...
4. PRIMARY KEY(기본키) : 고유 값, (NOT NULL + UNIQUE)모두 적용
...
VALUES (1111, '신민기', '천재1', '10');
COMMIT;

INSERT INTO EMP01 (EMPNO, ENAME, JOB, DEPTNO)
VALUES (2222, '신민아', '천재2', '40'); -- parent key not found // 
--DEPTE(ID)를 참조하는 DEPTNO 왜래키 값(40)이 DEPT 테이블에 없는 값이어서 부모키 값이 없다는 오류가 발생.
...
CREATE TABLE EMP02(
    ...
    PRIMARY KEY (EMPNO),  --기본키 설정 : PK
    FOREIGN KEY (DEPTNO) REFERENCES DEPT(ID) --외래키 설정 : FK
);
--===============================================================
...
INSERT INTO EMP03 (EMPNO, ENAME, JOB, DEPTNO)
VALUES (2222, '신민아', '천재2', '40');
--제약조건을 걸어서 생성 데이터를 제한 -오류 방지

--=============================================================================
/* 테이블에 제약조건 추가, 삭제 //                         좌측 테이블 우클릭해서 만들수도 있다.
-- 제약조건 추가 
...
ALTER TABLE EMP01 DROP CONSTRAINT SYS_C007035; -- CONSTRAINT NAME 입력.

ALTER TABLE EMP01 ADD CONSTRAINT EMP01_EMPNO_PK PRIMARY KEY (EMPNO);

-------------------------------------------------------------------------------
-- EMP02, EMP03 테이블 외래키 FOREIGN KEY 이름 변경처리 : SYS_C007039
-- EMP02 : EMP02_EMPNO_FK, EMP03 : FK_EMP03_DEPTNO

ALTER TABLE EMP02 RENAME CONSTRAINT SYS_C007039 TO EMP02_EMPNO_FK;
...
--외래키 DISABLE 상태인 경우 
INSERT INTO EMP02 VALUES (3333, '신민기3', '천재3', '40'); --참조키가 비활성화 됐으므로 40이 들어갈 수 있다. 따라서 제약조건이 필요하다.

--제약조건 활성화(적용) 또는 추가시
---- 제약조건 활성화(또는 추가시) 제약조건 위반 데이터가 있으면 활성화 불가능
-- ORA-02298 : cannot validate (MADANG.EMP02_EMPNO_FK) - parent keys not found
ALTER TABLE EMP02 ENABLE CONSTRAINT EMP02_EMPNO_FK; --이미 위반되는 데이터가 들어가 있으므로 제약조건 활성화가 불가능

--==============================================================================
-- 데이터사전 테이블
SELECT * FROM USER_CONS_COLUMNS; --유저가 접근할 수 있는 제약조건 
...
INSERT INTO HSCHOOL VALUES (1, 2, 1, '을지문덕'); 
