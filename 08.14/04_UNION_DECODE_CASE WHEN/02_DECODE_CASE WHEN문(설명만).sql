...
SELECT NAME, DECODE(NAME, '김연아', '야~김연아당!!')
FROM CUSTOMER
ORDER BY NAME
;
----------
SELECT NAME, DECODE(NAME, '김연아', '이야~ 김연아구마잉!', '누구세여')
FROM CUSTOMER
ORDER BY NAME
;
...
-- 이름이 '박지성'이면 '추구선수' 아니면 '운동선수'
SELECT NAME,
    DECODE(NAME, '박지성', '축구선수', '그냥 운동선수') AS DECODE_NAME,
    CASE NAME WHEN '박지성' THEN '축구선수' ELSE '그냥 운동선수' 
        END AS CASE_NAME
...
;
---------------------------------------------------------------
-- 가격 (PRICE)이 10000미만이면 '싸다', 10000~20000 : 적당하다
-- 20000보다 크면 '비싸다', 30000보다 크면 '너무 비싸다'
SELECT B.*
        CASE WHEN PRICE < 10000 THEN '싸따' ELSE
            CASE WHEN PRICE <= 20000 THEN '적당하다' ELSE
                CASE WHEN PRICE <= 30000 TEHN '비싸다'
                    ELSE '너무 비싸다'
                END
            END
        END AS "가격평가"
...
SELECT B.*
    CASE WHEN PRICE < 10000 THEN '싸따'
        WHEN PRICE <= 20000 TEHN '적당하다'
    END AS 평가
FROM BOOK B
ORDER BY PRICE
;
