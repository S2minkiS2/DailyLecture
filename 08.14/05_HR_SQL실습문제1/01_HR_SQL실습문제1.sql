...
FROM EMPLOYEES
WHERE TO_CHAR(HIRE_DATE, 'YYYY-MM-DD') LIKE '2008%'
;
--8) 월급여가 20000~30000 구간인 직원 사번, 성명(last_name first_name), 월급여 정보 조회
SELECT EMPLOYEE_ID, CONCAT(FIRST_NAME, LAST_NAME) AS FULL_NAME, SALARY
FROM EMPLOYEES
...
;
--13) 직종별 평균급여 이상인 직원 조회
SELECT E.EMPLOYEE_ID, E.SALARY, E.JOB_ID, E.FIRST_NAME
    FROM EMPLOYEES E INNER JOIN
    (SELECT JOB_ID, AVG(SALARY) AS AVGS FROM EMPLOYEES GROUP BY JOB_ID) TEST
    ON E.JOB_ID = TEST.JOB_ID
        AND E.SALARY >= TEST.AVGS
    ORDER BY TEST.JOB_ID
;

SELECT E.EMPLOYEE_ID, E.SALARY, A.* 
    FROM (SELECT JOB_ID, AVG(SALARY) AS AVG FROM EMPLOYEES GROUP BY JOB_ID) A INNER JOIN EMPLOYEES E 
