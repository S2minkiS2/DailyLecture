...
UPDATE BOOK
   SET BOOKNAME = 'A' || BOOKNAME --기존데이터를 읽어와서 붙이기 했다
     , PUBLISHER = 'IT_BOOK'
...
-- 동작은 BOOK 테이블 데이터가 삭제되면 BOOK_LOG 테이블에 이력을 남기기

create or replace TRIGGER AFTER_DELETE_BOOK
AFTER DELETE ON BOOK
FOR EACH ROW
BEGIN
    -- 삭제 로그를 북 로그에 남기기(딜리트 문)
    -- 딜리트 되면 북로그에 흔적을 남겨라
    INSERT INTO BOOK_LOG
        (BOOKID, BOOKNAME, PUBLISHER, PRICE, JOB_GUBUN)
...
--=======================
-- 트리거 비활성화 처리
ALTER TRIGGER AFTER_DELETE_BOOK DISABLE; --비활성화 : DELETE시 동작하지않음
