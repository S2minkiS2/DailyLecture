...
IF (조건식) THEN ~ ELSIF ~ END IF;
**********************/

create or replace PROCEDURE PRC_IF (
    IN_NUM IN NUMBER --파라미터 하나
)AS 
...
--====================
-- 반복문 : FOR, WHILE
--FOR 변수 IN 시작값 .. 최종값 // FOR 선언문
--          LOOP ~ END LOOP // 중괄호 역할
-- FOR문은 무조건 1씩 증가
------------------------------
-- 숫자(N) 하나로 입력받아서 합계 출력(1~N 까지의 합)
CREATE OR REPLACE PROCEDURE PRC_FOR_SUM (
    IN_NUM IN NUMBER
) 

AS V_SUM NUMBER := 0; --합계 저장할 변수 선언 (초기값 0 설정)
...
BEGIN
    ...
    -- 합계값 출력
    DBMS_OUTPUT.PUT_LINE('합계값 출력 : ' || V_SUM);
END;
--========================
/* LOOP ~ END LOOP; --무한반복문
LOOP
...
-- 입력값이 홀수면 홀수값만 더하고 //홀수면 더해라 (짝수면 ㅂㅂ)
-- 입력값이 짝수면 짝수값만 더해서 //짝수면 더해라
최종 합계값을 화면에 출력
<출력형태>
