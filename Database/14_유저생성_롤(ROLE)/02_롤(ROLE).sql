...
;
---------------------
--(SYSTEM) 롤(ROLE, 역할) 생성 : PROGRAMMER(개발자)라는 역할(롤) 생성
CREATE ROLE PROGRAMMER;

--(SYSTEM) PROGRAMMER 에게 권한 부여 테이블, 뷰(VIEW) 생성 권한 부여
GRANT CREATE ANY TABLE, -- ANY~ : 이 권한을 갖게되면 다른 유저에도 생성 가능
CREATE ANY VIEW TO PROGRAMMER;

--(SYSTEM) MDGUEST 유저에게 PROGRAMMER 롤을 부여 --> 어디든지 테이블, 뷰 생성 가능
...
dbms insert delete ... crud작업을 자바 코드로..
