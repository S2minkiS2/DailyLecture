...
-- 사용자(유저) 삭제 : DROP USER (연결중엔 DROP 불가)
DROP USER 유저명 [CASCADE];
--CASCADE 옵션은 삭제시점에 사용자(유저)가 보유한 모든 데이터 삭제
***********************/
...
-- (SYSTEM) 권한 부여 - MANAG.BOOK.을 SELECT 할 수 있는 권한을 MDGUEST 에게
SELECT * FROM MADANG.BOOK; --내가 가진 BOOK테이블이 아니니 MADANG.을 꼭 붙여야!
...
----------
--(MADANG) 권한 부여, 권한 회수( BOOK 테이블 SELECT 권한 MDGUEST EGG)
SELECT * FROM BOOK;
...
--------------
--(SYSTEM) 유저삭제 
DROP USER MDGUEST CASCADE; -- 삭제 권한이 있는 USER만 가능 (MADANG은 불가능)
