...
********************/
-- EMP01 테이블의 PRIMARY KEY 삭제 : SYS_C007043
...
ALTER TABLE EMP02 DROP CONSTRAINT SYS_C007056;
...
COMMIT;
SELECT * FROM EMP02;
--FOREIGN KEY 제약조건을 해제했기 때문에 
--없는 데이터인 '40'데이터가 입력 가능하다.

--제약조건 활성화 또는 추가 ENABLE
---- 제약조건 활성화 또는 추가시 제약조건 위반 데이터가 있으면 활성화 안됨
...
--=====================
-- 데이터사전 테이블
SELECT * FROM USER_CONS_COLUMNS; --마당 테이블 다 볼 수 있음
SELECT * FROM USER_CONSTRAINTS; --제약조건이 담긴 테이블

SELECT A.OWNER, A.TABLE_NAME, A.COLUMN_NAME, A.CONSTRAINT_NAME
     , B.CONSTRAINT_TYPE --P는 PRIMARY, R은 
