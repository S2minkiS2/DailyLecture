...
--커미션 있음
SELECT (FIRST_NAME ||' '|| LAST_NAME) AS NAME, 
        (SALARY + COMMISSION_PCT) AS "Salary + Commission"  
  FROM EMPLOYEES
 WHERE COMMISSION_PCT IS NOT NULL 
 ;
...
SELECT SUM(SALARY) SALARY_SUM, AVG(SALARY) SALARY_AVG, 
       MAX(SALARY) SALARY_MAX, MIN(SALARY) SALARY_MIN
  FROM EMPLOYEES
 WHERE  
GROUP BY JOB_ID
;
...
--사원들의 업무별 전체 급여 평균 구하기 > 10000
SELECT JOB_ID, AVG(SALARY) 
FROM EMPLOYEES
WHERE (SELECT AVG(SALARY) FROM EMPLOYEES GROUP BY JOB_ID) > 10000
GROUP BY JOB_ID
HAVING NOT IN (LIKE '%CLERK%')
...
SELECT (FIRST_NAME ||' '|| LAST_NAME) AS NAME, E.JOB_ID, 
...
SELECT DEPARTMENT_NAME
 ...
 WHERE E.DEPARTMENT_ID = D.DEPARTMENT_ID
   AND (SELECT COUNT(JOB_ID) FROM EMPLOYEES GROUP BY JOB_ID) >= 5
   ;
