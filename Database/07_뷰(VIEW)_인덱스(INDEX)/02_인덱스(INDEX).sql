...
<인덱스 재구성 문법> 
--수정 너무 많은 저장, 수정 시 중구난방됨 -> 재구성 삭제,추가,수정,다시생성
ALTER INDEX 인덱스명 REBUILD;
ALTER [UNIQUE] INDEX 인덱스명 [ON 테이블명 (컬럼명, ...)] REBUILD;
************************************/
-- 인덱스를 생성 : BOOK 테이블의 출판사(PUBLISHER) 컬럼에 IX_BOOK_PUB
CREATE INDEX IX_BOOK_PUB ON BOOK (PUBLISHER); --삭제해도 아래의 다른 인덱스가 적용될 수 있다.
SELECT * FROM BOOK ORDER BY PUBLISHER; 
SELECT * FROM BOOK WHERE PUBLISHER = '굿스포츠';  -- 인덱스 사용 검색

--똑같은 데이터 조회하는데 책 제목에는 인덱스가 없을 때
SELECT * FROM BOOK WHERE BOOKNAME = '축구의 이해'; --FULL SCAN

--이렇게 정렬된 것처럼 INDEX의 데이터도 정렬되어있다. PUBLISHER가 정렬된 듯이
--다이렉트로 접근할 수 있는 데이터를 따로 갖고있다.
--ORDERBY로 하면 일일이 모든 데이터를 스캔. 
--INDEX를 적용해서 사용하면 INDEX가 가나다순 정렬이 되어있다.
--다이렉트로 접근 가능한 ID값을 가지고있다.
--인덱스 적용해서 찾으면 굿스포츠 만났을 시 처리 (부분검색 처리)
--부분검색은 인덱스 정렬이 되어있기 때문에 가능하다.
--그냥 찾아도 찾아짐 BUT 데이터 많을 때 속도가 느려진다.


--인덱스 생성 : 2개 컬럼 기준
--- BOOK 테이블의 PUBLISHER, PRICE 컬럼을 대상으로 IX_BOOK_PUB_PRICE 인덱스 생성
CREATE INDEX IX_BOOK_PUB_PRICE ON BOOK (PUBLISHER, PRICE);
--첫번째 컬럼에 종속되어 두번째 컬럼이 정렬됨.
--첫번째가 아닌 두번째, 세번째 사용 시엔 인덱스 기능(순서)적용불가
--따라서 두번째 컬럼부턴 단독 사용 불가 (정렬X) 
--PUBLISHER에 종속되어 정렬된 PRICE

SELECT * FROM BOOK ORDER BY PUBLISHER, PRICE;
SELECT * FROM BOOK WHERE PUBLISHER = '굿스포츠'; -- 인덱스 사용
SELECT * FROM BOOK WHERE PRICE >= 7000; --인덱스 사용 안함(안됨)
...
SELECT * FROM BOOK WHERE PUBLISHER LIKE '%미디어'; --인덱스 사용 불가
                                        -->부분검색 사용 불가 : 처음부터 끝까지 다 찾아야..
                                        --> 인덱스가 쓸모없는 경우. 사용불가함. 안함.
SELECT * FROM BOOK WHERE PUBLISHER LIKE '%대한%'; -->인덱스 사용 불가

-- 원본 데이터를 가공하는 경우 (인덱스 적용안됨)
SELECT * FROM BOOK WHERE UPPER(PUBLISHER) = 'PEARSON'; --적용 못함
--인덱스 자체가 원본 데이터 기준으로 만들어지기 때문에 일치하는게 없어짐
---- 데이터변형 -> 인덱스 사용 불가 
...
(1) 다음 SQL 문을 수행해본다.
	SELECT name FROM Customer WHERE name LIKE '박세리'; //EQUAL
