/* 비교구문(분기처리) IF 문
IF (조건식) THEN ~ END IF;                  // IF와 END는 쌍으로 수를 맞춰서 코딩하세요.
...
BEGIN
    ...
    ELSIF (MOD(IN_NUM, 4) = 2) THEN
        DBMS_OUTPUT.put_line('2로 나눈 나머지 1');
    ELSE
        DBMS_OUTPUT.put_line('3로 나눈 나머지 1');
    END IF;
END;
--=================================================================================
-- 반복문 : FOR, WHILE
...
CREATE OR REPLACE PROCEDURE PRC_FOR_SUM (
    ...
    V_SUM NUMBER := 0; --합계 저장할 변수 선언, 초기값 0
BEGIN
    DBMS_OUTPUT.PUT_LINE('>>입력값 : ' || IN_NUM);
    --입력받은 숫자까지의 합계 구하기
...
--===================================================================================
/*
(실습) 숫자를 하나 입력 받아서 1 ~ 숫자까지의 합계 구하기, 중간에 홀짝도 기록하기?
...
BEGIN
    ...
    IF MOD(IN_NUM, 2) = 0 THEN
        DBMS_OUTPUT.PUT_LINE('입력숫자 : ' || IN_NUM || ', 짝수, 합계 : ' || Z_SUM);
    ELSE
        DBMS_OUTPUT.PUT_LINE('입력숫자 : ' || IN_NUM || ', 홀수, 합계 : ' || H_SUM);
    END IF;
END;
