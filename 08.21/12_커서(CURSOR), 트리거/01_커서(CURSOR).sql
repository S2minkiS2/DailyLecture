...
BEGIN
    ...
    LOOP 
        --3. 데이타추출(FETCH 커서명 INTO;)
        FETCH C_ORDERS 
        INTO V_ORDERID, V_BOOKNAME, V_CUSTNAME, V_SALEPRICE, V_ORDERDATE;
        
        EXIT WHEN C_ORDERS%NOTFOUND; --커서에 데이터가 없으면 반복을 종료(끝내기)
        
        --커서에서 가져온 데이터 화면출력
        DBMS_OUTPUT.PUT_LINE(V_ORDERID ||','|| V_BOOKNAME ||','|| V_CUSTNAME ||','|| 
