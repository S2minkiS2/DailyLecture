...
직렬화 (Serialization) / 역직렬화 (Deserialization)
 - 직렬화 : JVM 메모리에 있는 객체를 바이트스트림으로 변환하는 작업이다 (전송을 위해 데이터를 일렬로 세운다 생각해요)
 - 역직렬화 : 바이트스트림 데이터를 자바 객체로 변환하는 작업
--------------------------------------------------
데이터 전송을 위해서는 직렬화 처리가 가능해야 하는데, Serializable 형식만 가능
 - Serializable 인터페이스를 구현하면 된다.
데이터 직렬화 처리시 제외할 데이터는 transient 키워드로 지정한다. (return = null)
 - transient 처리
 transient 처리해도 전달되버리는 데이터 : static, final 처리된 데이터 
...
public class StudentVO implements Serializable {
	...
	//transient : Serializable 해서 데이터 전송시 값 전달 제외 
